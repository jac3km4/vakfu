...
impl Map {
    pub fn load<T: VkTexture, R: Indexed<i32, T>, S: Read + Seek>(queue: Arc<Queue>, reader: S, library: ElementLibrary, loader: &mut R) -> (Map, Vec<Arc<ImmutableImage<R8G8B8A8Srgb>>>) {
        ...
        for i in 0..len {
            ...
            match scan_fmt!(entry.name(), "{}_{}", i32, i32) {
                (Some(x), Some(y)) => {
                    ...
                    let patch = DecoderCursor::new(entry)
                        .decode::<RenderElementPatch>();
                    working_set.extend(patch.elements.iter()
                        .filter_map(|e|
                            library.elements.get(&e.display.element_id).map(|x| x.gfx_id)));
                }
                _ => ()
            }
        }
        ...
        let sprites =
            xys.iter().flat_map(|xy| {
                ...
                library
                    ...
                    .and_then(|element| {
                        pool.indices.get(&element.gfx_id).map(|desc|
                            Sprite::new(&spec, element, *desc)
                        )
                    })
            }).collect::<Vec<_>>();
